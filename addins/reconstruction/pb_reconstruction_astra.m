function pb_reconstruction_astra(recon_params)


%MODIFY FOR FFT SHIFT!! AS PER CENTRAL SLICE ASTRA
...
% center of detector
vectors(:,4) = 0-dsx*cos(angles)-recon_params.centre_shift(1)*cos(angles);
vectors(:,5) = 0-dsx*sin(angles)-recon_params.centre_shift(1)*sin(angles);
vectors(:,6) = (dsy);
...
proj_geom.DetectorRowCount = chunk_size;
...
if show_wb
    w = waitbar(0);
    set(w,'Name', 'TTreconstruction: parallel beam');
end
