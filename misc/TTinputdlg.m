function Answer=TTinputdlg(Prompt, Title, NumLines, DefAns, Resize)
...
if nargin==5 && isstruct(Resize)
  ...
  if isfield(Options,'Color'), FigColor=Options.FigColor; end
  if isfield(Options,'ButtonNames'), ButtonNames=Options.ButtonNames; end
end
...
if ishghandle(InputFig)
  ...
  Answer=cell(NumQuest+1,1);
  for lp=1:NumQuest,
      Answer(lp)=get(EditHandle(lp),{'String'});
  end
  Answer{end} = get(InputFig,'UserData');
  delete(InputFig);
else
  Answer={};
end
